...
{
    return ((HI_U8)((((HI_U32)color) >> pBit->offset) << (8 - pBit->length)) + /* 8 Digits to flip */
            ((HI_U8)(((HI_U32)(color) >> pBit->offset) << (8 - pBit->length)) >> pBit->length)); /* 8 Digits to flip */
}

static inline HI_S32 hifb_color2key(const struct fb_var_screeninfo *pVar, HI_S32 color)
{
    if (pVar->bits_per_pixel <= 8) { /* 8 left shift */
        return color;
    } else {
        HI_U8 r, g, b;
        ...
        return (r << 16) + (g << 8) + b; /* left shift altitude 16 8 */
    }
}
#endif
...
typedef struct {
    ...
    /* whether support a certain layer, for example:x5 HD support HIFB_SD_0 not support HIFB_SD_1 */
...
HI_S32 hifb_init(hi_void *pArgs);
#endif
