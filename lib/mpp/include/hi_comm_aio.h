...
typedef enum hiAI_CHN_MODE_E {
    AI_CHN_MODE_NORMAL = 0,     /* normal mode for AI chn */
    AI_CHN_MODE_FAST,           /* fast mode for AI chn */
    AI_CHN_MODE_BUTT,
} AI_CHN_MODE_E;

typedef struct hiAI_CHN_ATTR_S {
    AI_CHN_MODE_E enMode;       /* operating mode */
} AI_CHN_ATTR_S;
...
typedef struct hiAI_AEC_CONFIG_S {
    ...
    HI_S8 s8NearAllPassEnergy;      /* the far-end energy threshold for judging whether unvarnished transmission:
                                       0 -59dBm0, 1 -49dBm0, 2 -39dBm0, recommend 1 */
    HI_S8 s8NearCleanSupEnergy;     /* the energy threshold for compelling reset of near-end signal:
                                       0 12dB, 1 15dB, 2 18dB, recommend 2 */

    HI_S16 s16DTHnlSortQTh;         /* the threshold of judging single or double talk, recommend 16384, [0, 32767] */

    HI_S16 s16EchoBandLow;          /* voice processing band1, low frequency parameter,
                                       [1, 63) for 8k, [1, 127) for 16k, recommend 10 */
    HI_S16 s16EchoBandHigh;         /* voice processing band1, high frequency parameter, (s16EchoBandLow, 63] for 8k,
                                       (s16EchoBandLow, 127] for 16k, recommend 41,
                                       s16EchoBandHigh must be greater than s16EchoBandLow */
    HI_S16 s16EchoBandLow2;         /* voice processing band2, low frequency parameter,
                                       [1, 63) for 8k, [1, 127) for 16k, recommend 47 */
    HI_S16 s16EchoBandHigh2;        /* voice processing band2, high frequency parameter,
                                       (s16EchoBandLow2, 63] for 8k, (s16EchoBandLow2, 127] for 16k,
                                       recommend 72, s16EchoBandHigh2 must be greater than s16EchoBandLow2 */

    HI_S16 s16ERLBand[6];           /* ERL protect area, [1, 63] for 8k, [1, 127] for 16k,
                                       frequency band calculated by s16ERLBand * 62.5,
    ...
    HI_S16 s16VioceProtectFreqL1;   /* protect area of near-end low frequency1, (s16VioceProtectFreqL, 63] for 8k,
                                       (s16VioceProtectFreqL, 127] for 16k, recommend 6 */
...
typedef struct hiAUDIO_HPF_CONFIG_S {
    HI_BOOL bUsrMode;   /* mode 0: auto mode 1: mannual. */
    AUDIO_HPF_FREQ_E enHpfFreq; /* freq to be processed, value:HiFiVqe/TalkVqe/AoVqe(80Hz, 120Hz, 150Hz),
                                   RecordVqe(80Hz) */
} AUDIO_HPF_CONFIG_S;
...
typedef struct hiAI_DRC_CONFIG_S {
    ...
    HI_S16  s16AttackTime;   /* time of signal change from large to small (ms),
                                range:HiFiVqe[10, 250]ms, RecordVqe[10, 126]ms */
    HI_S16  s16ReleaseTime;  /* time of signal change from small to large (ms),
                                range:HiFiVqe[10, 250]ms, RecordVqe[10, 126]ms */

    HI_S16 s16OldLevDb[VQE_DRC_SECNUM];  /* old curve level(dB), default[0, -472, -792, -960, -1280],
                                            range:[-1440, 0]ms,store from big to small,scale:Q4 */
    HI_S16 s16NewLevDb[VQE_DRC_SECNUM];  /* new curve level(dB), default[0, -174, -410, -608, -1021],
                                            range:[-1440, 0]ms, store from big to small,scale:Q4 */
} AI_DRC_CONFIG_S;

typedef struct hiAI_TALKV2_EQ_CONFIG_S {
    HI_BOOL bUsrMode;                       /* mode 0: auto, mode 1: mannual. */

    HI_S8  s8GaindB[TALKVQEV2_EQ_BAND_NUM]; /* EQ band, range:[-50, 20]dB */
} AI_TALKV2_EQ_CONFIG_S;

typedef struct hiAI_TALKV2_PNR_CONFIG_S {
    HI_BOOL bUsrMode;           /* mode 0: auto, mode 1: mannual. */

    HI_S16 s16MinGainLimit;     /* maximum allowable noise rejection, default:5827, range:[0, 32767] */
    HI_S16 s16SnrPriorLimit;    /* minimum prior signal-to-noise ratio, default:3276, range:[0, 32767] */
    HI_S16 s16HtThreshold;      /* voice decision threshold, default:10, range:[0, 80] */
    HI_S16 s16HsThreshold;      /* harmonic detection threshold, default:100, range:[100, 1100] */
    HI_S16 s16AlphaPh;          /* speech probability smoothing coefficient, default:90, range:[0, 100] */
    HI_S16 s16AlphaPsd;         /* noise estimation smoothing factor, default:65, range:[0, 100] */
    HI_S16 s16PriorSnrFixed;    /* fixed prior signal-to-noise ratio, default:30, range:[1, 499] */
    HI_S16 s16CepThreshold;     /* cepstrum smooth speech threshold, default:16, range:[0, 100] */
    HI_S16 s16SpeechProtectThreshold;   /* speech protect threshold, default:90, range:[0, 100] */
    HI_S16 s16HemEnable;        /* harmonic enhancement enable, default:1, range:[0, 1] */
    HI_S16 s16TcsEnable;        /* cepstral smoothing switch, default:0, range:[0, 1] */

    HI_S16 s16Reserved[3];      /* 3: reserved */
} AI_TALKV2_PNR_CONFIG_S;

typedef struct hiAI_TALKV2_NR_CONFIG_S {
    HI_BOOL bUsrMode;           /* mode 0: auto, mode 1: mannual. */

    HI_S16 s16MinGainLimit;     /* maximum allowable noise rejection, default:3276, range:[0, 32767] */
    HI_S16 s16SnrPriorLimit;    /* minimum prior signal-to-noise ratio, default:327, range:[0, 32767] */
    HI_S16 s16HtThreshold;      /* voice decision threshold, default:10, range:[0, 80] */
    HI_S16 s16HsThreshold;      /* harmonic detection threshold, default:100, range:[100, 1100] */
    HI_S16 s16PriNoiseEstiMode; /* single mic noise estimation algorithm mode, default:0, range:[0, 2] */
    HI_S16 s16PriorSnrFixed;    /* fixed prior signal-to-noise ratio, default:20, range:[1, 499] */
    HI_S16 s16PrioSnrThreshold; /* instantaneous prior snr, default:7, range:[5, 20] */
    HI_S16 s16SmPrioSnrThreshold; /* smooth prior snr, default:10, range:[8, 30] */

    HI_S16 s16Reserved[16];     /* 16: reserved */
} AI_TALKV2_NR_CONFIG_S;

typedef struct hiAI_TALKV2_AGC_CONFIG_S {
    HI_BOOL bUsrMode;           /* mode 0: auto, mode 1: mannual. */

    HI_S8 s8TargetLevel;        /* target level, default:-16, range:[-120, 120] */
    HI_U8 u8MaxGain;            /* maximum gain, default:80, range:[0, 240] */
    HI_S8 s8MinGain;            /* minimum gain, default:-60, range:[-120, 0] */
    HI_S8 s8AdjustSpeed;        /* adjustable speed, default:5, range: [1, 10] */

    HI_S16 s16Reserved[3];      /* 3: reserved */
} AI_TALKV2_AGC_CONFIG_S;

/* Defines the configure parameters of TalkV2 VQE. */
typedef struct hiAI_TALKVQEV2_CONFIG_S {
    HI_U32            u32OpenMask;

    HI_S32            s32WorkSampleRate;    /* sample rate:16kHz */
    ...
    HI_BOOL bReserved0;
    HI_S16  s16Reserved1[19];   /* 19: reserved */
    HI_U8   u8Reserved2[8];     /* 8: reserved */
...
#define HI_ERR_AIO_SYS_NOTREADY      HI_DEF_ERR(HI_ID_AIO, EN_ERR_LEVEL_ERROR, EN_ERR_SYS_NOTREADY)
/* aio is working */
#define HI_ERR_AIO_BUSY              HI_DEF_ERR(HI_ID_AIO, EN_ERR_LEVEL_ERROR, EN_ERR_BUSY)
/* vqe err */
