 ...
 * Description: Function of hi_ae_comm.h
 * Author: ISP SW
 * Create: 2012/06/28
 */
...
typedef struct hiAE_SENSOR_DEFAULT_S {
    ...
    HI_BOOL bMaxTimeStepEnable;
    HI_U32  u32MaxIncTimeStep[WDR_MAX_FRAME_NUM];
    HI_U32  u32MaxDecTimeStep[WDR_MAX_FRAME_NUM];
    ...
    ISP_IRIS_F_NO_E  enMaxIrisFNO;  /* RW; Range:[0, 10]; Format:4.0;
                                       Max F number of Piris's aperture, it's related to the specific iris */
    ISP_IRIS_F_NO_E  enMinIrisFNO;  /* RW; Range:[0, 10]; Format:4.0;
    ...
    ISP_QUICK_START_PARAM_S stQuickStart;
    ISP_PRIOR_FRAME_E enPriorFrame;
    HI_BOOL bAEGainSepCfg;
    HI_BOOL bLhcgSupport;
...
typedef struct hiAE_EXP_PARAM_REG_S {
    HI_U32 u32RegAddr;
    HI_U32 u32RegValue;
} AE_EXP_PARAM_REG_S;

typedef struct hiAE_EXP_PARAM_S {
    HI_U32 u32TarFps;
    HI_U32 u32ExpTime;
    HI_U32 u32ExpAgain;
    HI_U32 u32ExpDgain;
    HI_U32 u32ExpIspDgain;
    AE_EXP_PARAM_REG_S stTimeReg[10]; /* 10 */
    AE_EXP_PARAM_REG_S stAgainReg[10]; /* 10 */
    AE_EXP_PARAM_REG_S stDgainReg[10]; /* 10 */
} AE_EXP_PARAM_S;
...
typedef struct hiAE_SENSOR_EXP_FUNC_S {
    ...
    HI_VOID (*pfn_cmos_get_inttime_max)(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable,
        HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime);

    /* long frame mode set */
    HI_VOID (*pfn_cmos_ae_fswdr_attr_set)(VI_PIPE ViPipe, AE_FSWDR_ATTR_S *pstAeFSWDRAttr);
    HI_VOID (*pfn_cmos_ae_quick_start_status_set)(VI_PIPE ViPipe, HI_BOOL bQuickStartStatus);
    HI_VOID (*pfn_cmos_exp_param_convert)(VI_PIPE ViPipe, AE_EXP_PARAM_S *pstExpFuncs);
