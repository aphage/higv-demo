 ...
 * Description: Function of hi_awb_comm.h
 * Author: ISP SW
 * Create: 2012/06/28
 */
...
typedef struct hiAWB_DBG_STATUS_S {
    ...
    HI_U32 au32HistInfo[256];   /* 256 is size of hist info */
...
typedef struct hiAWB_CCM_TAB_S {
    ...
    HI_U16 au16CCM[CCM_MATRIX_SIZE];       /* RW;  Range: [0x0, 0xFFFF]; Format:8.8;
...
typedef struct hiAWB_AGC_TABLE_S {
    ...
    HI_U8   au8Saturation[ISP_AUTO_ISO_STRENGTH_NUM];   /* RW;adjust saturation, different iso with different
                                                           saturation */
} AWB_AGC_TABLE_S;
